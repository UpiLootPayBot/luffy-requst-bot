...
async def copy_channel_messages_to_user(
    ...
    FIX:
    This uses ONLY get_forward_source_channel_id().
    It never falls back to CHANNEL_ID or activity_channel_id, so messages are copied
    only from the host/source channel selected in settings/env.
    """
    ...
    for msg_id in FORWARD_MSG_IDS:
        try:
            ...
            sent += 1
            logger.info(f"copy_message OK host_source={source_id} mid={msg_id} → uid={user_id}")
            await asyncio.sleep(0.4)
        except BadRequest as e:
            logger.warning(f"copy_message failed host_source={source_id} mid={msg_id} → uid={user_id}: {e}")
        except TelegramError as e:
            logger.warning(f"copy_message failed host_source={source_id} mid={msg_id} → uid={user_id}: {e}")
        except Exception as e:
            logger.warning(f"copy_message unexpected host_source={source_id} mid={msg_id} → uid={user_id}: {e}")

    if sent == 0:
        logger.error(
            f"Host copy FAILED. source={source_id}, msg_ids={FORWARD_MSG_IDS}, user={user_id}. "
...
async def _show_settings(q, ctx):
    ...
    text = (
        ...
        f"{E_INFO} Host Source Only : <code>{get_forward_source_channel_id()}</code>\n"
